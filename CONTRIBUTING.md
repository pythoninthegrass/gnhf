...
- Node 20+, TypeScript, ESM-only. See `AGENTS.md` for agent instructions and architecture notes.
- Use TDD for bug fixes and new features. Unit tests are co-located as `*.test.ts`; e2e tests live under `e2e/`.
- Run `npm run lint`, `npm run format:check`, `npm run typecheck`, and `npm test` before pushing. The pipeline will run them again, but a fast local pass saves rounds.
